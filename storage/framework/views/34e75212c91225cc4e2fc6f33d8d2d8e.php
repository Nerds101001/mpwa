<div class="tab-pane fade show" id="buttonmessage" role="tabpanel">
	<form class="row g-3" action="<?php echo e(route('messagetest')); ?>" method="POST">
		...
		<label for="messageb" class="form-label"><?php echo e(__('Message')); ?></label>
		...
		<button type="button" id="addbutton" class="btn btn-outline-primary btn-sm mr-2 mt-4"><?php echo e(__('Add Button')); ?></button>
    ...
    $(document).ready(function() {
        ...
        add_button.on('click', function(e) {
            ...
            if (countButtons() >= max_fields) {
                if (window.toastr) toastr['warning']('<?php echo e(__("Maximal 3 button")); ?>');
                return;
            }
            var idx = seq++;
            var buttonForm =
                '<div class="form-group buttoninput mt-3 border rounded p-3" id="buttonGroup'+idx+'">'+
                    '<div class="d-flex justify-content-between align-items-center mb-2">'+
                        '<label for="buttonType'+idx+'" class="form-label mb-0">'+("<?php echo e(__('Button :x Type', ['x' => ':x'])); ?>".replace(':x', idx))+'</label>'+
                        '<button type="button" class="btn btn-outline-danger btn-sm d-inline-flex align-items-center removeButton" data-index="'+idx+'">'+
                            '<i class="ti tabler-x me-1"></i><?php echo e(__("Remove")); ?>'+
                        '</button>'+
                    '</div>'+
                    ...
                    '</select>'+
                    '<label for="buttonDisplayText'+idx+'" class="form-label mt-2"><?php echo e(__("Display Text")); ?></label>'+
        ...
        $(document).on('change', '.buttonType', function() {
            ...
            if (selectedType === 'call') {
                additionalFields.append(
                    '<label for="phoneNumber'+index+'" class="form-label"><?php echo e(__("Phone Number")); ?></label>'+
                    '<input type="text" name="button['+index+'][phoneNumber]" class="form-control" id="phoneNumber'+index+'" required>'
                );
            } else if (selectedType === 'url') {
                additionalFields.append(
                    '<label for="url'+index+'" class="form-label"><?php echo e(__("URL")); ?></label>'+
                    '<input type="text" name="button['+index+'][url]" class="form-control" id="url'+index+'" required>'
                );
            } else if (selectedType === 'copy') {
                additionalFields.append(
                    '<label for="copyText'+index+'" class="form-label"><?php echo e(__("Copy Text")); ?></label>'+
...
<?php /**PATH C:\laragon\www\whatsapp\resources\themes/vuexy/views/ajax/test/formbutton.blade.php ENDPATH**/ ?>